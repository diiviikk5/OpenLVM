...
export async function GET(request: NextRequest) {
  ...
  const unauth = requireAuthenticated(ctx, "Arena integration hub");
  if (unauth) return unauth;
  try {
    const data = await runWorkbenchBridge("arena_integration_hub", [ctx.actorId]);
    if (typeof data === "object" && data && "error" in data) {
      const detail = String((data as { error: string }).error);
      return contextError("Arena integration hub failed", ctx, workbenchErrorStatus(detail), detail);
    }
