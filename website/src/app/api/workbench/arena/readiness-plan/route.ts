...
export async function GET(request: NextRequest) {
  ...
  const unauth = requireAuthenticated(ctx, "Arena readiness plan");
  if (unauth) return unauth;
  try {
    const data = await runWorkbenchBridge("arena_readiness_plan", [ctx.actorId]);
    if (typeof data === "object" && data && "error" in data) {
      const detail = String((data as { error: string }).error);
      return contextError("Arena readiness plan failed", ctx, workbenchErrorStatus(detail), detail);
    }
